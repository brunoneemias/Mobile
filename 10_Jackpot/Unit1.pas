...
begin
  ...
  begin
    ...
    Showmessage('Você ganhou R$' + floattostr(ganho));
  end
    ...
    begin
      saldo:= saldo - aposta;
      txtSaldo.Text := FloatToStr(saldo);
      Showmessage('Você perdeu R$' + floattostr(aposta));
    end;
...
try //Tratamento de erro
    ...
    if(test=1) then
     begin
      Showmessage('O valor de saldo é inválido!');
       txtSaldo.Opacity := 1;
       txtSaldo.SetFocus;
     end
     else if (test=2) then
      begin
        Showmessage('O valor da aposta é inválido!');
        txtAposta.SetFocus;
      end;
  end;
